 ...
 *   http://www.pololu.com/docs/0J19,int flanco_sens, int keep_track, int noise_threshold
 *
 ...
 */
#include <EEPROMex.h> //Incluir libreria para guardar datos de calibracion en la EEPROM
...
class QTRSensors
{
  public:
    ...
    bool recordCalibration(); //Devuelve un 1 lógico si se guardaron lso valores adecuadamente
    ...
    // Limpia la EEPROM DE LOS VALORES CALIBRADOS
    bool clearEEPROM(char led = 30); //Devuelve un 1 lógico cuando termian de borrar los valores

    void calibration(int time,unsigned char readMode=QTR_EMITTERS_ON ,char led = 30);
    ...
    int readLine(unsigned int *sensor_values, unsigned char readMode = QTR_EMITTERS_ON, unsigned char white_line = 0,int flanco_sens=0, int keep_track=200, int noise_threshold=50);
