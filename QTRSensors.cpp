...
/* Libreria modificada con fines didacticos y Open Source respetando la
licencia y derecho de autor de (c) 2008-2012 Pololu

By: Michael Vargas
Contact me: msvargas97@gmail.com
*/
//NOTA ACTIVAR DEPURADOR O VISUALIZADOR DE DATOS GUARDADOS Y RESTAURADOS
#define MICHAEL_DEBUG 0  // Activar log para saber si esta guardando y restaurando la calibración, escriba 1 para activar
...
bool QTRSensors::recordCalibration(){

  for (int i = 0; i < _numSensors; i++) //Guardar los valores minimos y maximos a la vez
  {                                                     //Gracias al uso de punteros por parte de la libreria se puede realizar lo siguiente
    record=(EEPROM.writeInt(i*2,calibratedMinimumOn[i]));  //Guardar los valores en la EEPROM cada 2 slots gracias a la libreria
    record=(EEPROM.writeInt((i*2)+(_numSensors*2),calibratedMaximumOn[i])); //ya que en Arduino el int es de 2 Bytes y la EEMPROM almacena 1 byte por cada slot
  }

#if MICHAEL_DEBUG
  Serial.print("Se utilizaron ");
    Serial.print(((_numSensors-1)*2)+(_numSensors*2));
     Serial.print(" espacios o casillas de la EEPROM, puedes almacenar valores apartir +2 en adelante");
     Serial.println();
     delay(3000);
  Serial.print("Valores Minimos guardados en EEPROM-> ");
  for (int i = 0; i < _numSensors; i++){
    Serial.print( QTR_EEPROM.readInt(i*2));
    Serial.print(' ');
  }
  Serial.println("");
  ...
  for (int i = 0; i < _numSensors; i++){
    Serial.print( QTR_EEPROM.readInt((i*2)+(_numSensors*2)));
    Serial.print(' ');
  }
  Serial.println("");
   Serial.println("Libreria modificada By Michael Vargas, prueba RW ( OK ) MOD 08/06/2015");
      Serial.println("Creditos EEPROMMex");
        Serial.println("Vamos a reiniciar los valores de calibracion y luego restaurarlos con la EEPROM:");

#endif
       return record;
}
bool  QTRSensors::restoreCalibration(){
 bool restore;
 calibrate(QTR_EMITTERS_OFF);
 resetCalibration(); // Borrar valores old
#if MICHAEL_DEBUG
    Serial.print("Restaurando");
    Serial.println(" ");
    #endif
     for (int i = 0; i < _numSensors; i++)
  {
     ...
     Serial.print("Value minimum:");
     Serial.println( QTR_EEPROM.readInt(i*2));
     Serial.print("Value maximum:");
     Serial.println(QTR_EEPROM.readInt((i*2)+(_numSensors*2)));
     delay(2000);
     #endif
    calibratedMinimumOn[i] = EEPROM.readInt(i*2); //Asignar los valores guardados en la EEPROM en los valores de calibracion
    calibratedMaximumOn[i] = EEPROM.readInt((i*2)+(_numSensors*2));
  }
  ...
  if(restore)
  {
  Serial.println("Se restauraron los valores de calibracion con exito!");
    Serial.println();
    Serial.print("Valores Minimos restaurados desde la EEPROM-> ");
  for(int i = 0; i < _numSensors; i++){
    Serial.print(calibratedMinimumOn[i]);
    Serial.print(' ');
  }
  Serial.println("");
  Serial.print("Valores Maximos  restaurados desde la EEPROM-> ");
  ...
  for (int i = 0; i < _numSensors; i++){
    ...
    if(!restore)
    {
      Serial.println("No se restauraron los valores de calibracion, vuelvelo a intentar :(");
    Serial.println();
    }
...
bool QTRSensors::clearEEPROM(char led){
if(led != 30){
        pinMode(led,OUTPUT);
     digitalWrite(led, HIGH);
}
  for (int i = 0; i < ((_numSensors*4)-1); i++){
    EEPROM.write(i, 0);

#if MICHAEL_DEBUG
  Serial.println("Datos de calibracion borrados de la EEPROM con exito!");
#endif
}
if(led != 30){
...
void QTRSensors::calibration(int time,unsigned char readMode,char led) { //funcion para calibrar sensores
  if(led != 30){
        pinMode(led,OUTPUT);
        digitalWrite(led, HIGH);
}
